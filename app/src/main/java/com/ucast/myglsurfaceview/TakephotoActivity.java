...
//        Toast.makeText(this,p.toString(),Toast.LENGTH_LONG).show();
        ...
        for (int i = 1; i <= num ; i++) {
            ...
            boxMsg.setLedId(i + "");
            boxMsg.setShowPic(Environment.getExternalStorageDirectory().getPath() + "/Ucast/port" + (i%3 + 1)+ ".jpg");
            boxMsg.setShowPic(true);
            boxs.put(boxMsg.getLedId(),boxMsg);
        }

//        ApManager.openHotspotWithNoPassword(this,"ucast_ap");
    }
...
//                        BackgroundCamera.getInstance().takePhotoWithParam(one.getLedId(), true);
    ...
    public void startARActivity(StartArEvent event){
        isTaking = false;
        int l  = boxs.size();
        ToastUtil.showToast(TakephotoActivity.this,"拍照完成-》" + boxs.get("1").getPointInScreen());
        startArAct();
    }

    public void startArAct(){
        Intent intent = new Intent(TakephotoActivity.this,UnityActivity.class);
